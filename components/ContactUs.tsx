...
import { Mail, Phone, MapPin, Send, Globe, Briefcase, GraduationCap, Clock, ChevronDown } from 'lucide-react';
...
const ContactUs: React.FC = () => {
      ...
      try {
        ...
        if (mapContainerRef.current && !mapInstanceRef.current && window.L) {
          const map = window.L.map(mapContainerRef.current, {
            center: [lat + 0.002, lng], // Offset slightly so popup is centered initially
            zoom: 17,
            zoomControl: false,
          ...
          window.L.tileLayer('https://mt0.google.com/vt/lyrs=m&x={x}&y={y}&z={z}', {
          ...
          const iconHtml = `
            <div class="relative w-16 h-16 flex items-center justify-center hover:scale-110 transition-transform cursor-pointer group">
                ...
                <div class="absolute -bottom-1 w-4 h-2 bg-black/20 rounded-[100%] blur-sm"></div>
            </div>
          `;
          ...
          const popupContent = `
            <div class="font-sans text-left">
                <div class="flex items-center gap-4 mb-4">
                    <div class="w-12 h-12 rounded-xl bg-brand-navy flex items-center justify-center shadow-md overflow-hidden border border-gray-100 shrink-0">
                        <img src="/images/company-logo.png" alt="Logo" class="w-full h-full object-cover" />
                    </div>
                    <div class="flex-1 min-w-0">
                        <h3 class="text-lg font-bold text-brand-navy leading-tight m-0 mt-1">Our Headquarters</h3>
                        <p class="text-brand-teal font-medium text-xs m-0">Long Vacation (S) Pte Ltd</p>
                    </div>
                </div>

                <div class="flex items-start gap-3 text-gray-600 bg-gray-50 p-3 rounded-xl mb-4 border border-gray-100">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#FF796C" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mt-1 shrink-0"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
                    <p class="leading-snug font-medium text-sm m-0 -mt-0.5">
                ...
                <a 
                    ...
                    target="_blank"
                    class="flex items-center justify-center gap-2 w-full bg-brand-green !text-black font-bold py-3 rounded-xl hover:opacity-90 transition-all shadow-md no-underline text-sm"
                >
  ...
  const departments = [
      { name: 'Inbound Tours', email: 'inbound@longvacationtravel.com', phone: '+65 6776 0491', icon: Globe },
      { name: 'Outbound Tours', email: 'outbound@longvacationtravel.com', phone: '+65 6776 0492', icon: Send },
      { name: 'General Enquiry', email: 'enquiry@longvacationtravel.com', phone: '+65 6776 0496', icon: MapPin },
      { name: 'Partnerships', email: 'partners@longvacationtravel.com', phone: '+65 6776 0493', icon: Briefcase },
      { name: 'Corporate MICE', email: 'mice@longvacationtravel.com', phone: '+65 6776 0494', icon: Briefcase },
      { name: 'Educational', email: 'edu@longvacationtravel.com', phone: '+65 6776 0495', icon: GraduationCap },
  ];
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      ...
      {/* Dynamic Map Section */}
      <div className="w-full h-[500px] md:h-[600px] bg-[#e5e3df] relative shadow-inner z-0">
        {!isMapLoaded && (
             <div className="absolute inset-0 flex items-center justify-center bg-gray-100 text-gray-400 font-bold">
                 Loading Map...
             </div>
        )}
        <div ref={mapContainerRef} className="w-full h-full z-0" style={{ isolation: 'isolate' }}></div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <form className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input type="text" placeholder="John Doe" className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-all font-bold text-brand-navy placeholder:font-normal placeholder:text-gray-400" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-brand-navy uppercase tracking-wider">Email Address</label>
                                <input type="email" placeholder="john@example.com" className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-all font-bold text-brand-navy placeholder:font-normal placeholder:text-gray-400" />
                            </div>
                        </div>

                        <div className="space-y-2">
                            <label className="text-sm font-bold text-brand-navy uppercase tracking-wider">Subject</label>
                            ...
                            <div className="relative" ref={dropdownRef}>
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-xl bg-gray-50 border flex items-center justify-between transition-all font-bold text-brand-navy cursor-pointer ${
                                        isDropdownOpen ? 'border-brand-teal ring-1 ring-brand-teal' : 'border-gray-200'
                                    }`}
                                >
                                    <span>{subject}</span>
                                            ...
                                            <ul className="py-2">
                                                {subjects.map((item) => (
                                                    <li key={item}>
                                                        ...
                                                        >
                                                            <span className={`font-medium text-sm ${subject === item ? 'text-brand-navy' : 'text-gray-600'}`}>
                                                                {item}
                                                            </span>
                                                            {subject === item && (
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea rows={5} placeholder="Tell us more about your travel plans..." className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-all font-bold text-brand-navy placeholder:font-normal placeholder:text-gray-400"></textarea>
                        </div>
            ...
            <div className="lg:col-span-5 space-y-8">
                ...
                <div className="bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
                    <h3 className="font-bold text-xl text-brand-navy mb-6">Department Directory</h3>
                        ...
                        {departments.map((dept, idx) => (
                            <div key={idx} className="grid grid-cols-[auto_1fr] gap-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <a href={`tel:${dept.phone}`} className="flex items-center gap-2 text-sm text-gray-500 hover:text-brand-coral transition-colors hover:underline">
                ...
                <div className="bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="font-bold text-xl text-brand-navy">Operating Hours</h3>
                    </div>
                    
                    <div className="space-y-4 text-sm text-gray-600">
                        <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                            <span className="font-medium">Monday - Friday</span>
                            <span className="font-bold text-brand-navy">10:00 AM - 7:00 PM</span>
                        </div>
                        <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                            <span className="font-medium">Saturday</span>
                            <span className="font-bold text-brand-navy">10:00 AM - 2:00 PM</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="font-medium">Sunday & PH</span>
                            <span className="font-bold text-brand-coral bg-brand-coral/10 px-3 py-1 rounded-full text-xs uppercase tracking-wider">Closed</span>
                        </div>
                    </div>
                </div>
            </div>
